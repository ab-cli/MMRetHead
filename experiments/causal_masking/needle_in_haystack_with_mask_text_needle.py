...
from mmrethead.data import head_mask_load_text_task
...
def default_task_data_path(dataset_name, max_context_len):
    name = (dataset_name or "mm_text").lower()
    if any(key in name for key in ["mm_text", "retrieval-text", "text_needle_image"]):
        return DEFAULT_MMLONGBENCH_DATA_ROOT / "NIAH" / f"retrieval-text_test_K{context_length_to_k(max_context_len)}_dep6.jsonl"
    if "nq" in name or "doc_qa" in name or "longdocurl" in name or "mmlongdoc" in name or "slidevqa" in name:
        raise ValueError("--task_data_path is required for this non-MM-NIAH task.")
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        self.sandwich_needle = sandwich_needle
        ...
        self.load_data(sandwich_needle=sandwich_needle)
    ...
    def init_head_mask_with_save_name(self, head_score_path):
        ...
        if self.sandwich_needle:
            self.save_name += "_sandwich-needle"
    ...
    def decode_from_prefill(self, inputs, decode_len, block_list=None):
        prefill_inputs = {k: v[:, :-1] if k in ["input_ids", "attention_mask"] else v for k, v in inputs.items()}
    ...
    def load_data(self, sandwich_needle=False):
        max_context_length = max(self.context_lengths)
        
        result = head_mask_load_text_task(
            self.task_data_path,
            self.task_image_dir,
        ...
        if sandwich_needle:
            ...
            self.test_item["needle"] = {"type": "text", "text": sandwich_info["needle"]}
            self.test_item["question"] = sandwich_info["retrieval_question"]
            self.test_item["answer"] = sandwich_info["real_needle"]
            self.metric = lambda ans, pred: scorer.score(ans, pred)['rouge1'].recall * 100
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_data_path', type=str, default=None, help='path to one task JSONL; defaults to repo-local MMLongBench MM-NIAH text file inferred from --max_context_len')
    ...
    parser.add_argument('--dataset_name', type=str, default="mm_text", help='optional dataset name for loader dispatch')
    ...
    parser.add_argument('--sandwich_needle', type=ast.literal_eval, choices=[True, False], default=False, help="use the needle: The best thing to do in San Francisco is to eat a sandwich in Dolores Park.")
    ...
    ht = LLMNeedleHaystackTester(
        ...
        sandwich_needle=args.sandwich_needle,
    ...
    if not example_ids:
        ...
        for index, example_id in enumerate(example_ids, start=1):
            ...
            try:
                ...
                ht.example_idx = -1
                ht.load_data(sandwich_needle=args.sandwich_needle)
                ht.testing_results = []
