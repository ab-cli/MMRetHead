...
from .cache_with_query import DynamicCacheWithQuery


if not getattr(_masking_utils, "_is_torch_greater_or_equal_than_2_6", True):
    # Gemma3 passes image-token overlay mask functions through the Transformers mask
    # helpers. With FlashAttention2 those helpers do not materialize the custom
    # mask, but Transformers 5.0.0rc0 still blocks torch<2.6 before reaching that
    # path. The experiment environment is torch 2.5.1, so relax the guard here.
    _masking_utils._is_torch_greater_or_equal_than_2_6 = True
...
class Gemma3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Modified for query caching
        if past_key_values is not None:
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}  # Specific to RoPE models
            if isinstance(past_key_values, DynamicCacheWithQuery):
                # For use in computing attention scores
                query_states_to_cache = query_states[:, :, past_key_values._query_indices, :]
                key_states, value_states = past_key_values.update(query_states_to_cache, key_states, value_states,
        ...
        # Modified to add attention head blocking
        # pop block_list from kwargs
        block_list = kwargs.pop('block_list', None)
        if block_list is not None:
          assert self.config._attn_implementation == "flash_attention_2", "Only flash_attention_2 is supported when using block_list"      
...
class Gemma3Model(Gemma3PreTrainedModel):
    ...
    def __init__(self, config: Gemma3Config):
        ...
        # modified to use custom gemma model
        language_model = Gemma3TextModel._from_config(config=config.text_config)
