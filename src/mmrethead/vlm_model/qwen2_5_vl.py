...
class Qwen2VLModel(LLM):
    def __init__(
        ...
        self.processor.vision_start_token = "<|vision_start|>"
        self.processor.vision_end_token = "<|vision_end|>"
        ...
        # See this issue https://github.com/huggingface/transformers/issues/35588
        # There is a conflict between dynamo and flash_attn. Dynamo requires 'max_seqlen_q' to be a tensor while flash_attn requires it to be an integer.
    ...
    def expand_visual_tokens(self, text, inputs):
        ...
        if 'pixel_values_videos' in inputs:
            ...
            index = 0
            for i in range(len(text)):
                while self.processor.video_token in text[i]:
                    num_video_tokens = inputs["video_grid_thw"][index].prod() // merge_length
                    text[i] = text[i].replace(self.processor.video_token, "<|placeholder|>" * num_video_tokens, 1)
                    index += 1
                text[i] = text[i].replace("<|placeholder|>", self.processor.video_token)
        return text
    ...
    # Modified : now shortened with new build prompt function and also have null score calibration
    ...
    # Modified : Added. This incorporates the custom dynamic cache with query
    ...
    # Modified to make use of the new functions created
    @torch.no_grad()
